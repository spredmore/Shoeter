...
namespace customAnimation
{
    class Shoes : Character
    {
        ...
        private bool bouncingVertically = false;
        public bool delayCollision = true;  // We need to let the Guy travel a little bit before Shoes can catch him.
        float delayCollisionInterval = 0.3f;  // Amount of time needed to elapse before the Guy and Shoes collide. Measured in milliseconds.
        int counter = 1;
        int limit = 1;
        ...
        public void Update(GameTime gameTime, ref Guy guy)
        {
            ...
            //debug = "bouncingVertically: " + bouncingVertically.ToString();
            //debug2 = "isJumping: " + isJumping.ToString();
        ...
        private void handleMovement(GameTime gameTime, ref Guy guy)
        {
            ...
            // ******************************************************
            // Set the vecocity based on if the Shoes are on the ground or in the air.
            // ******************************************************
            if (isJumping == true)  velocity.X = airMovementSpeed;
            else                    velocity.X = groundMovementSpeed;

            // ******************************************************
            // The player wants to jump.
            // The velocity is set to a negative number so that the Shoes will move upwards.
            // ******************************************************
            if (isJumping == false && ((newKeyboardState.IsKeyDown(up) && !oldKeyboardState.IsKeyDown(up)) || (newKeyboardState.IsKeyDown(Keys.Space) && !oldKeyboardState.IsKeyDown(Keys.Space))) && standingOnGround() == true && underTile() == false && guy.tileAbove() == false)
            {
                isJumping = true;
            ...
            // ******************************************************
            if (newKeyboardState.IsKeyDown(right) && delayCollision == false)
            {
            ...
            if (newKeyboardState.IsKeyDown(left) && delayCollision == false)
            {
                bouncingHorizontally = 0;
                currentTime = 0;
                position.X -= velocity.X * delta;
            ...
            // ******************************************************
            if (isJumping == true)                  doPlayerJump(delta);
            else handleFalling(delta); // Handles for when the player walks off the edge of a platform.
        ...
        private void handleFalling(float delta)
        {
            ...
            {
                // Determine if the Shoes has fallen onto a spring.
                // If so, allow the Shoes to bounce. Otherwise, stop falling.
                setTileArrayCoordinates(this.position.X, this.position.Y);
                if (Level.tiles[(int)this.TileArrayCoordinates.X, (int)this.TileArrayCoordinates.Y].TileRepresentation == 'S' && velocity.Y > 4f)
                {
